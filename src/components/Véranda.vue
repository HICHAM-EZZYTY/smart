<template>
    ...
    <div class="Véranda block m-0 m-auto grid grid-cols-1 gap-4 mb-6 w-4/5 md:w-3/4  md:grid-cols-9 lg:w-3/5">
        <div class="inpt1 relative w-2/5 md:w-full  md:col-span-2">
                ...
                <div v-for="(item, index) in items" :key="index" class="salledebainoptions md:col-span-7 grid gap-4 grid-cols-1 md:grid-cols-2 ">
          <div class="flex w-full">
					<input oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="field_name" class="inptt border border-2 rounded-r px-4 py-2 w-full" type="text" :placeholder="'Surface véranda '+[[index+1]]"/>
