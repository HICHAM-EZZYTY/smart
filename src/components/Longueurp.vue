    ...
    <div class="Longueur block m-0 m-auto mb-8 w-4/5 md:w-3/4 lg:w-3/5 ">
            <div class="inpts grid grid-cols-1 gap-4 w-full mt-3 md:grid-cols-2">
                 <div class="flex w-full">
                    <input oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="field_name" class="espaceinpt border border-2 rounded-r px-4 py-2 w-full" type="text" placeholder="Longueur de péniche " />
                    <span class="text-sm border border-2 rounded-l px-4 py-2  whitespace-no-wrap">m</span>
                </div>

                <div class="flex w-full">
                    <input oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="field_name" class="espaceinpt inpt border border-2 rounded-r px-4 py-2 w-full" type="text" placeholder="lorgeur de péniche" />
